...
通常来说，一个map对象是由复数个相关联的键值对组合而成的。键和值都可以是任意类型，每一个键只会在一个map中出现一次，但是同样的值可以出现多次，Dart支持用Dart字符值和Map类来`提供/产出/声明`map变量。
...
:::tip
你可能以为会用`new Map()`而不是`Map()`来创建Map对象，从Dart<Badge text="2.0"/>开始，`new`关键词是可选择的。更多信息参阅[构造方法使用](https://dart.dev/guides/language/language-tour#using-constructors)
:::

#### 像在JavaScript里一样以键值的形式对来向一个map对象里添加项
